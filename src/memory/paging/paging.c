...
static int paging_get_indexes(void* v_addr, uint32_t* directory_index_out, uint32_t* table_index_out)
{
    int res = 0;
    ...
    *directory_index_out = ((uint32_t)v_addr / (PAGING_TOTAL_ENTRIES_PER_TABLE * PAGING_PAGE_SIZE));
    *table_index_out = ((uint32_t)v_addr % (PAGING_TOTAL_ENTRIES_PER_TABLE * PAGING_PAGE_SIZE) / PAGING_PAGE_SIZE);
...
int paging_set(uint32_t* directory, void* v_addr, uint32_t val)
{
    ...
    int res = paging_get_indexes(v_addr, &directory_index, &table_index);
    if (res < 0)
    {
        return res;
    }
