...
static inline bool heap_validate_alignment(void* ptr)
{
...
static uint32_t heap_align_to_upper_value(uint32_t val)
{
...
static inline int heap_get_entry_type(HEAP_BLOCK_TABLE_ENTRY entry)
{
    return entry & 0x0f;
}
...
static inline void* heap_block_to_address(struct heap* heap, int block) 
{
...
{
    size_t aligned_size = heap_align_to_upper_value(size);
    uint32_t total_blocks = aligned_size / HEAP_BLOCK_SIZE;

    return heap_malloc_blocks(heap, total_blocks);
}

static inline int heap_address_to_block(struct heap* heap, void* addr)
{
